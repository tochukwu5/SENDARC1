...
const NAV_ITEMS = [
  ...
  { id: 'send', label: 'Send Money', icon: '💸', path: '/send' },
  ...
  return (
    <aside className="w-60 flex-shrink-0 bg-[#0D1117] border-r border-[#1e2530] min-h-screen flex flex-col">
      ...
      <nav className="flex-1 p-4 space-y-1">
        <p className="text-[10px] tracking-widest text-[#556] mb-2 px-2">MAIN</p>
        {NAV_ITEMS.map(n => (
          <Link key={n.id} to={n.path}
            className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-['Space_Grotesk'] font-medium transition-all ${
              active === n.id ? 'bg-[#0a2030] text-[#00D4FF] border border-[#00D4FF]/30' : 'text-[#8892a0] hover:text-white hover:bg-[#0f1822]'
            }`}
          >
        ...
        <p className="text-[10px] tracking-widest text-[#556] mb-2 px-2 mt-6">ACCOUNT</p>
        {ACCOUNT_ITEMS.map(n => (
          <Link key={n.id} to={n.path}
            className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-['Space_Grotesk'] font-medium transition-all ${
              active === n.id ? 'bg-[#0a2030] text-[#00D4FF] border border-[#00D4FF]/30' : 'text-[#8892a0] hover:text-white hover:bg-[#0f1822]'
      ...
      {wallet && (
        <div className="p-4 border-t border-[#1e2530]">
          <div className="bg-[#0f1822] border border-[#1e2530] rounded-xl p-3">
            <p className="text-[10px] tracking-widest text-[#556] mb-1">CONNECTED WALLET</p>
            ...
            <button onClick={() => { disconnect(); navigate('/') }} className="text-[10px] text-red-400 hover:underline mt-2 block">Disconnect</button>
          </div>
        </div>
      )}
    </aside>
    ...
    return (
      <div className="min-h-screen bg-[#0D1117] flex items-center justify-center text-center px-6">
        <div>
          ...
          <Link to="/connect" className="bg-[#00D4FF] text-[#0D1117] font-['Space_Grotesk'] font-bold px-6 py-3 rounded-xl">Connect Wallet</Link>
        </div>
      </div>
    )
  }
      ...
      <main className="flex-1 p-8">
        ...
        <div className="flex justify-between items-start mb-8">
          <div>
            ...
            <p className="text-[#8892a0] text-sm mt-1">Welcome back — here's your transfer activity</p>
          </div>
          <Link to="/send" className="bg-[#00D4FF] text-[#0D1117] font-['Space_Grotesk'] font-bold px-5 py-2.5 rounded-xl hover:opacity-90 transition-all text-sm">
            + Send Money
          </Link>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {[
            { label: 'TOTAL SENT', value: '$2,840', sub: '↑ +$340 this month', subColor: 'text-green-400' },
            { label: 'TRANSACTIONS', value: '24', sub: 'All confirmed on Arc', subColor: 'text-[#8892a0]' },
            { label: 'FEES PAID', value: '$0.072', sub: 'vs $142 via bank wire', subColor: 'text-[#8892a0]' },
            { label: 'COUNTRIES SENT TO', value: '4', sub: 'NG · GH · KE · ZA', subColor: 'text-[#8892a0]' },
          ].map(s => (
            <Card key={s.label} className="p-5">
              <p className="text-[10px] tracking-widest text-[#8892a0] mb-2">{s.label}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 mb-6">
          ...
          <Card className="lg:col-span-2 p-5">
            <p className="text-sm font-semibold font-['Space_Grotesk'] mb-4">BY COUNTRY</p>
            <div className="space-y-4">
              {[
                { flag: '🇳🇬', name: 'Nigeria', pct: 68 },
                { flag: '🇬🇭', name: 'Ghana', pct: 18 },
                { flag: '🇰🇪', name: 'Kenya', pct: 9 },
                { flag: '🇿🇦', name: 'S. Africa', pct: 5 },
              ].map(c => (
                <div key={c.name}>
                  <div className="flex justify-between text-sm mb-1.5">
                    <span className="flex items-center gap-2"><span>{c.flag}</span><span className="text-white font-['Space_Grotesk'] font-semibold">{c.name}</span></span>
                    <span className="text-[#00D4FF] font-bold">{c.pct}%</span>
                  </div>
                  <div className="h-1.5 bg-[#1e2530] rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-[#00D4FF] to-[#00FFCC] rounded-full" style={{ width: `${c.pct}%` }} />
                  </div>
                </div>
              ))}
            </div>
        ...
        <Card className="overflow-hidden">
          <div className="flex justify-between items-center px-5 py-4 border-b border-[#1e2530]">
            <p className="text-sm font-semibold font-['Space_Grotesk']">RECENT TRANSACTIONS</p>
            <Link to="/dashboard/transactions" className="text-xs text-[#00D4FF] hover:underline">View All →</Link>
          </div>
